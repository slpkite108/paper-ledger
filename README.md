...
로그인 토큰과 사용 중인 API 키는 탭 메모리에만 유지합니다. Google에 저장한 데이터는 해당 계정의 앱 전용 저장소를 사용합니다. 프리셋은 논문 행을 저장하지 않으므로 편집 결과는 Excel/CSV로 보관하세요. API 키·클라이언트 비밀값·토큰은 저장소에 넣지 마세요.
