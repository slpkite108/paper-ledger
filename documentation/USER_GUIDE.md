...
일반 Drive 문서 접근 권한은 요청하지 않습니다. 토큰은 탭의 메모리에만 유지하며, 새로고침·만료 후에는 다시 연결합니다. Google 저장 키는 Google 접근 제어와 HTTPS로 보호하며 앱 자체의 추가 비밀번호 암호화는 없습니다. 저장된 키·즐겨찾기·프리셋은 앱에서 각각 삭제할 수 있습니다. 브라우저에 저장했던 항목은 Google 연결 후 다시 저장하면 해당 계정으로 옮길 수 있습니다.
