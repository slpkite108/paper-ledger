...
import { issnSummary, paperIdentifiers, setManualIssn } from '@/lib/journal-identifiers';
...
export default function Home() {
  const [layout, setLayout] = useState<Layout>(()=>{const initial=defaultLayout();try{if(localStorage.getItem('paper-ledger-date-basis')==='online')initial.dateBasis='online';}catch{}return initial;}); const [exporting, setExporting] = useState(false);
  ...
  function restoreFavorite(favorite: Favorite) {
    ...
    setLayout(prev=>({...prev,dateBasis:p.dateBasis}));
  ...
  const recognizedPapers = useMemo(() => papers.map(p => automaticallyRecognize(withDateBasis(p,layout.dateBasis), layout.criteria)), [papers, layout.criteria, layout.dateBasis]);
  ...
  function openEdit(p: Paper) { ++editSeq.current; setEditing({ ...withDateBasis(p,layout.dateBasis), values: { ...withDateBasis(p,layout.dateBasis).values } }); setEditError(''); setEnriching(false); }
