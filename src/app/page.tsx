...
export default function Home() {
  ...
  return <div className="app-shell">
    ...
    <div className="workspace"><aside className="search-panel">
      ...
      <div className="page-heading"><div><div className="step-label">02 / PUBLICATION RECORDS</div><h2>연구실적 정리</h2><p>여러 저자의 논문을 모아, 참여교수별 실적 양식으로 정리하세요.</p></div><div className="export-actions"><LayoutSettings layout={layout} onChange={setLayout} sample={visiblePapers[0]} onMessage={setNotice}/><Button variant="outline" disabled={!selectedPapers.length || exporting} onClick={download}><Download size={17}/>CSV</Button><Button className="export-button" disabled={!selectedPapers.length || exporting} onClick={() => void downloadExcel()}>{exporting ? <Loader2 size={17} className="animate-spin"/> : <Download size={17}/>}Excel 다운로드{selectedPapers.length > 0 && <span className="export-count">{count(selectedPapers.length)}</span>}</Button></div></div>
      ...
      <section className="records"><div className="active-layout"><span>현재 양식 · <strong>{layout.name}</strong></span><small>정렬과 열 설정은 양식·프리셋에서 저장할 수 있습니다.</small></div><SortControls layout={layout} onChange={setLayout}/><Tabs defaultValue="list"><div className="records-toolbar"><TabsList><TabsTrigger value="list">논문 목록</TabsTrigger><TabsTrigger value="form">내 양식 표 ({layout.columns.length})</TabsTrigger></TabsList><div className="record-actions"><CriteriaPanel layout={layout} onLayout={setLayout} papers={selectedPapers} onApply={applyCriteria}/><Button variant="outline" size="sm" disabled={loading || classifying || !papers.some(p => p.doi && !p.arxiv)} onClick={() => classifyPapers(papers)}>Crossref 학술유형 보완</Button><span>{count(selectedPapers.length)}건 선택됨</span></div></div>
