...
import { AuthorIdentityReview, AuthorIdentityDetails } from '@/components/author-identity-review';
...
import { applyPublicationDates, dateSummary, dateComparison, dateResolution, dateBasisLabels, withDateBasis, type DateBasis, normalizedDoi, setManualPublicationDate } from '@/lib/publication-dates';
import { withIssnPreference, issnSummary, paperIdentifiers, setManualIssn } from '@/lib/journal-identifiers';
...
export default function Home() {
  const [layout, setLayout] = useState<Layout>(()=>{const initial=defaultLayout();try{if(localStorage.getItem('paper-ledger-date-basis')==='online')initial.dateBasis='online';initial.preferIssnL=localStorage.getItem('paper-ledger-prefer-issn-l')==='true';}catch{}return initial;}); const [exporting, setExporting] = useState(false);
  useEffect(()=>{try{localStorage.setItem('paper-ledger-date-basis',layout.dateBasis);}catch{}},[layout.dateBasis]);
  useEffect(()=>{try{localStorage.setItem('paper-ledger-prefer-issn-l',String(layout.preferIssnL));}catch{}},[layout.preferIssnL]);
  ...
  function restoreFavorite(favorite: Favorite) {
    ...
    setLayout(prev=>({...prev,dateBasis:p.dateBasis,preferIssnL:p.preferIssnL}));
  ...
  async function classifyPapers(targets: Paper[]) {
    const candidates = targets.filter(p => p.authorIdentity?.status !== 'excluded' && p.doi && !p.arxiv && !p.supplementalSource);
  ...
  const recognizedPapers = useMemo(() => papers.map(p => automaticallyRecognize(withIssnPreference(withDateBasis(p,layout.dateBasis),layout.preferIssnL), layout.criteria)), [papers, layout.criteria, layout.dateBasis, layout.preferIssnL]);
  ...
  async function loadPapers(more = false) {
    ...
    for (const a of targets) {
      ...
      try {
        ...
        setApplied(range);
        const fromOpenAlex = data.works.map(w => { const p = toPaper(w, a, professorNames[a.id] ?? a.display_name); const cached = paperCache.current.get(p.id); return cached ? { ...cached, authorIdentity: p.authorIdentity, values: { ...cached.values, professor: professorNames[a.id]?.trim() || a.display_name } } : p; });
        ...
        setSelected(prev => new Set([...prev, ...next.filter(p => p.authorIdentity?.status !== 'excluded').map(p => p.id)]));
  ...
  function openEdit(p: Paper) { ++editSeq.current; setEditing(withIssnPreference(withDateBasis(p,layout.dateBasis),layout.preferIssnL)); setEditError(''); setEnriching(false); }
  function closeEdit() { ++editSeq.current; setEditing(null); setEnriching(false); }
  function setValue(key: FieldKey, value: string) { setEditing(prev => prev ? key === 'published' ? setManualPublicationDate(prev, value) : key === 'issn' ? setManualIssn(prev,value) : { ...prev, ...(key === 'category' ? { recognitionCategoryManual: true } : {}), ...(key === 'venue' ? { venueManual: true } : {}), values: { ...prev.values, [key]: value } } : null); }
  ...
  return <div className="app-shell">
    ...
    <nav className="research-navigation paper-navigation" aria-label="검색 페이지"><a href="#/papers" aria-current="page">논문 검색</a><a href="#/patents">국내 특허 검색 <ArrowUpRight size={15}/></a></nav>
    <div className="workspace"><aside className="search-panel">
      ...
      <div className="publication-options"><label><Checkbox checked={excludeArxiv} onCheckedChange={v => setExcludeArxiv(v === true)}/>arXiv 제외</label><label title="연도는 범위에 걸치지만 월을 확인할 수 없는 자료를 함께 표시합니다. 전체 연도가 범위 안이면 항상 포함합니다."><Checkbox checked={includeUnknownMonths} onCheckedChange={v => setIncludeUnknownMonths(v === true)}/>기간 경계의 월 미상 포함</label><label><Checkbox checked={mergeLatest} onCheckedChange={v => setMergeLatest(v === true)}/>같은 제목·같은 유형 최신 1편</label><div className="kind-filter"><label htmlFor="kind-filter">학술유형</label><Select value={publicationKind} onValueChange={v => setPublicationKind(v as PublicationKind | 'all')}><SelectTrigger id="kind-filter"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="all">전체 유형</SelectItem>{Object.entries(kindLabels).map(([k, label]) => <SelectItem key={k} value={k}>{label}</SelectItem>)}</SelectContent></Select></div><JournalSearch key={journalRun} automaticResults={journalResults} existingPapers={papers} dateBasis={layout.dateBasis} authors={chosenAuthors} professorNames={professorNames} sources={journalSources} onSources={changeJournalSources} range={{from,to,fromMonth,toMonth}} includeUnknownMonths={includeUnknownMonths} onAdd={addJournalPapers} disabled={loading||classifying}/><AuthorFavorites payload={{ preferIssnL:layout.preferIssnL, dateBasis:layout.dateBasis, journalSources, authors: chosenAuthors, professorNames: Object.fromEntries(chosenAuthors.map(a => [a.id, professorNames[a.id] || a.display_name])), from, to, fromMonth, toMonth, includeUnknownMonths, excludeArxiv, mergeLatest, publicationKind }} onRestore={restoreFavorite} disabled={loading}/></div>
      <section className="date-basis-options" aria-label="ISSN 적용 기준"><label htmlFor="issn-basis">ISSN 기준</label><select id="issn-basis" value={layout.preferIssnL?'linking':'first'} onChange={e=>setLayout(prev=>({...prev,preferIssnL:e.target.value==='linking'}))}><option value="first">목록의 첫 번째 ISSN</option><option value="linking">ISSN-L 우선</option></select><p>기본값은 원본 목록의 첫 번째 ISSN입니다. ISSN-L 우선을 선택했는데 ISSN-L이 없으면 첫 번째 번호를 사용합니다. 표·정렬·CSV·Excel에 즉시 반영하고 즐겨찾기·프리셋에 저장합니다. 직접 수정한 ISSN은 유지합니다.</p></section>
      <section className="date-basis-options" aria-label="출판일 적용 기준"><label htmlFor="date-basis">저널 출판일 기준</label><select id="date-basis" value={layout.dateBasis} disabled={loading||classifying} onChange={e=>setLayout(prev=>({...prev,dateBasis:e.target.value as DateBasis}))}><option value="issue">권·호 발행일 우선</option><option value="online">온라인 게재일 우선</option></select><p>두 날짜가 모두 있으면 선택한 기준, 하나만 있으면 확인된 날짜를 사용합니다. 종류가 구분되지 않은 출판일은 별도로 표시합니다. 변경 즉시 목록·기간 필터·정렬·CSV·Excel에 적용됩니다. 학술대회는 개최일을 유지합니다.</p>{papers.length>0&&<p role="status">불러온 저널 기준: 선택 기준 적용 {recognizedPapers.filter(p=>dateResolution(p)==='preferred').length}건 · 다른 날짜로 대체 {recognizedPapers.filter(p=>dateResolution(p)==='fallback').length}건 · 종류 미확인 {recognizedPapers.filter(p=>dateResolution(p)==='unspecified').length}건 · 날짜 미확인 {recognizedPapers.filter(p=>dateResolution(p)==='missing').length}건 · 직접 지정 예외 {recognizedPapers.filter(p=>dateResolution(p)==='manual').length}건</p>}</section>
      ...
      <AuthorIdentityReview papers={recognizedPapers}/>
