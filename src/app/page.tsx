...
import { dateSummary, normalizedDoi, setManualPublicationDate } from '@/lib/publication-dates';
...
export default function Home() {
  ...
  async function classifyPapers(targets: Paper[]) {
    const candidates = targets.filter(p => p.doi && !p.arxiv);
  ...
  const visiblePapers = sortPapers(visiblePublications(papers, { excludeArxiv, mergeLatest, publicationKind }), layout);
  ...
  async function loadPapers(more = false) {
    ...
    for (const a of targets) {
      ...
      try {
        ...
        next.forEach(p => paperCache.current.set(p.id, p)); typeCandidates.push(...next.filter(p => !p.publicationDates?.crossrefChecked));
  ...
  function setValue(key: FieldKey, value: string) { setEditing(prev => prev ? key === 'published' ? setManualPublicationDate(prev, value) : { ...prev, values: { ...prev.values, [key]: value } } : null); }
  ...
  async function enrich() {
    ...
    try { const data = await getData<CrossrefData>('/api/crossref?' + new URLSearchParams({ doi: editing.doi })); if (seq !== editSeq.current) return;
      setEditing(prev => prev ? applyCrossref(prev, data, true) : prev); setEditError('빈 서지정보와 자동 출판일을 확인했습니다. 직접 지정한 날짜는 유지됩니다. 날짜 근거를 확인한 뒤 변경사항을 반영하세요.');
  ...
  return <div className="app-shell">
    ...
    <div className="workspace"><aside className="search-panel">
      ...
      <p className="table-hint">DOI 문헌의 출판일·학술유형을 Crossref로 자동 확인합니다. 날짜 아래에서 근거를 확인하고, 논문 편집에서 출판사·인쇄·온라인 날짜를 비교하세요. 연도만 등록된 자료는 월 미상으로 표시합니다. 조회 연도는 OpenAlex 수록 연도 기준이므로 보완된 출판일과 다를 수 있습니다.</p>
      {!visiblePapers.length ? <Empty className="paper-empty"><EmptyHeader><EmptyMedia className="paper-empty-icon"><BookOpen size={34}/></EmptyMedia><EmptyTitle>{loading ? '학술정보를 불러오고 있습니다' : papers.length ? '현재 필터에 맞는 문헌이 없습니다' : loaded ? '해당 기간에 수록된 문헌이 없습니다' : chosenAuthors.length ? '조회할 기간을 정하고 논문을 불러오세요' : '논문 목록이 여기에 표시됩니다'}</EmptyTitle><EmptyDescription>{loading ? workingAuthor + '의 저자와 출판정보를 확인하는 중입니다.' : papers.length ? '학술유형이나 arXiv 제외 조건을 바꾸거나 다음 논문을 불러오세요.' : loaded ? '조회 기간을 넓히거나 다른 저자 프로필을 확인해 보세요.' : '제목, 저자, 출판정보를 가져온 뒤 원하는 양식에 맞춰 실적 항목을 편집할 수 있습니다.'}</EmptyDescription></EmptyHeader>{loading && <Loader2 className="animate-spin text-primary"/>}</Empty> : <>
      ...
      <div className="bottom-note"><Info size={16}/><p>SCIE·BK 인정 구분, IF, 연구보조원 수, 주저자 여부, 기여율, 사사 기관 수는 직접 확인해 입력하세요. 조회 결과에는 논문 외 문헌도 포함될 수 있습니다. 공동 논문은 참여교수별로 한 행씩 표시되며, 총 건수도 교수별 행 기준입니다. <strong>조회 결과는 최대 1시간 캐싱됩니다. 논문 편집 내용은 이 탭에서만 유지됩니다. 저자 선택과 조회 조건은 즐겨찾기로 저장할 수 있습니다. 작업 후 Excel/CSV를 저장하세요.</strong></p></div>
    </main></div>
