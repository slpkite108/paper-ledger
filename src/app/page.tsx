...
export default function Home() {
  ...
  const visiblePapers = sortPapers(visiblePublications(papers, { excludeArxiv, mergeLatest, publicationKind, range: applied, includeUnknownMonths }), layout);
  ...
  async function loadPapers(more = false) {
    ...
    for (const a of targets) {
      ...
      try {
        ...
        next.forEach(p => paperCache.current.set(p.id, p)); typeCandidates.push(...next.filter(p => !p.publicationDates?.crossrefChecked || (supportsPublisherDate(p.doi) && !p.publicationDates?.publisherChecked)));
  ...
  return <div className="app-shell">
    ...
    <div className="workspace"><aside className="search-panel">
      ...
      <div className="publication-options"><label><Checkbox checked={excludeArxiv} onCheckedChange={v => setExcludeArxiv(v === true)}/>arXiv 제외</label><label title="연도는 범위에 걸치지만 월을 확인할 수 없는 자료를 함께 표시합니다. 전체 연도가 범위 안이면 항상 포함합니다."><Checkbox checked={includeUnknownMonths} onCheckedChange={v => setIncludeUnknownMonths(v === true)}/>기간 경계의 월 미상 포함</label><label><Checkbox checked={mergeLatest} onCheckedChange={v => setMergeLatest(v === true)}/>같은 제목은 최신 1편만</label><div className="kind-filter"><label htmlFor="kind-filter">학술유형</label><Select value={publicationKind} onValueChange={v => setPublicationKind(v as PublicationKind | 'all')}><SelectTrigger id="kind-filter"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="all">전체 유형</SelectItem>{Object.entries(kindLabels).map(([k, label]) => <SelectItem key={k} value={k}>{label}</SelectItem>)}</SelectContent></Select></div><AuthorFavorites payload={{ authors: chosenAuthors, professorNames: Object.fromEntries(chosenAuthors.map(a => [a.id, professorNames[a.id] || a.display_name])), from, to, fromMonth, toMonth, includeUnknownMonths, excludeArxiv, mergeLatest, publicationKind }} onRestore={restoreFavorite} disabled={loading}/></div>
      <p className="filter-description">arXiv 제외는 주 출처가 arXiv인 문헌에 적용합니다. 다른 곳에 정식 게재된 문헌은 유지합니다. 동일 제목은 불러온 문헌에서 참여교수별로 최신 출판일 1편을 표시하며, 기간·유형 필터를 먼저 적용합니다.</p>
