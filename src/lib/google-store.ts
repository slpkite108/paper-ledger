...
export function disconnectGoogle() { accessToken = ''; expiresAt = 0; currentUser = null; notify(); }
...
async function googleRequest<T>(path: string, init: RequestInit = {}): Promise<T> {
  if (!accessToken || Date.now() >= expiresAt) throw new Error('Google 연결이 만료되었습니다. 검색 설정에서 Google 계정으로 다시 연결하세요.');
  ...
  if (response.status === 401) { expiresAt = 0; throw new Error('Google 연결이 만료되었습니다. Google 계정으로 다시 연결하세요.'); }
  ...
  return new Promise((resolve, reject) => {
    const client = api.initTokenClient({ client_id: id, scope: 'openid email profile ' + scope,
      ...
      callback: async token => {
        ...
        accessToken = token.access_token; expiresAt = Date.now() + Math.max(0, (Number(token.expires_in) || 3600) - 60) * 1000;
        try {
          const profile = await googleRequest<{ sub?: string; email?: string; name?: string }>('/oauth2/v3/userinfo');
          if (!profile.sub || !profile.email) throw new Error('Google 계정 정보를 확인하지 못했습니다.');
          currentUser = { sub: profile.sub, email: profile.email, name: profile.name || profile.email, hasSavedKey: false }; notify(); resolve(currentUser);
        } catch (error) { disconnectGoogle(); reject(error); }
      },
    });
...
async function files(kind: 'favorite' | 'key' | 'preset') {
  ...
  do {
    ...
    const page = await googleRequest<{ files?: DriveFile[]; nextPageToken?: string }>('/drive/v3/files?' + params); results.push(...page.files || []); cursor = page.nextPageToken || '';
  } while (cursor && results.length < 500);
  return results;
}
async function readFile<T>(id: string) { return googleRequest<T>('/drive/v3/files/' + encodeURIComponent(id) + '?alt=media'); }
async function writeFile(kind: 'favorite' | 'key' | 'preset', value: unknown, id?: string) {
  if (id) return googleRequest('/upload/drive/v3/files/' + encodeURIComponent(id) + '?uploadType=media', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(value) });
  ...
  return googleRequest('/upload/drive/v3/files?uploadType=multipart', { method: 'POST', headers: { 'Content-Type': 'multipart/related; boundary=' + boundary }, body });
}
export async function readGoogleFavorites() {
  const stored = await files('favorite'); const favorites: Favorite[] = [];
  // Bounded sequential reads avoid burst requests and preserve API errors.
  for (const file of stored) favorites.push(favoriteSchema.parse(await readFile(file.id)));
...
export async function createGoogleFavorite(name: string, payload: FavoritePayload) {
  if ((await files('favorite')).length >= 50) throw new Error('Google 즐겨찾기는 50개까지 저장할 수 있습니다.');
  const favorite = favoriteSchema.parse({ id: crypto.randomUUID(), name, payload, createdAt: Date.now() });
  await writeFile('favorite', favorite); return favorite;
}
export async function deleteGoogleFavorite(id: string) {
  const stored = await files('favorite'); const matches = stored.filter(f => f.appProperties?.favoriteId === id);
  for (const file of matches) await googleRequest('/drive/v3/files/' + encodeURIComponent(file.id), { method: 'DELETE' });
}
export async function readGoogleKey() {
  const stored = await files('key');
  const data = stored[0] ? await readFile<{ apiKey?: string }>(stored[0].id) : null;
...
export async function saveGoogleKey(apiKey: string) {
  ...
  const stored = await files('key'); await writeFile('key', { apiKey }, stored[0]?.id);
  if (currentUser) { currentUser = { ...currentUser, hasSavedKey: true }; notify(); }
}
export async function deleteGoogleKey() {
  for (const file of await files('key')) await googleRequest('/drive/v3/files/' + encodeURIComponent(file.id), { method: 'DELETE' });
  if (currentUser) { currentUser = { ...currentUser, hasSavedKey: false }; notify(); }
}
export async function readGoogleLayouts() {
  const owner = currentUser?.sub; const stored = await files('preset'); const result: Layout[] = [];
  for (const file of stored) { if (currentUser?.sub !== owner) throw new Error('계정이 변경되었습니다. 다시 불러오세요.'); result.push(layoutSchema.parse(await readFile(file.id))); }
  return result;
}
export async function saveGoogleLayout(value: Layout) {
  const owner = currentUser?.sub; const layout = layoutSchema.parse(value); const stored = await files('preset');
  ...
  if (currentUser?.sub !== owner) throw new Error('계정이 변경되었습니다. 다시 저장하세요.');
  await writeFile('preset', layout, existing?.id);
}
export async function deleteGoogleLayout(id: string) {
  const owner = currentUser?.sub; const stored = await files('preset');
  for (const file of stored.filter(f => f.appProperties?.presetId === id)) {
    if (currentUser?.sub !== owner) throw new Error('계정이 변경되었습니다. 다시 시도하세요.');
    await googleRequest('/drive/v3/files/' + encodeURIComponent(file.id), { method: 'DELETE' });
  }
}
