...
export const MAX_REFERENCE_ROWS = 30_000;
export const referenceSchema = z.object({ id:z.string().min(1).max(100), name:z.string().trim().min(1).max(100), year:z.string().regex(/^\d{4}$/), url:z.string().url().refine(v=>/^https?:\/\//.test(v)), rows:z.array(referenceRowSchema).min(1).max(MAX_REFERENCE_ROWS) });
...
export const builtinMjl:Reference={id:'scie-mjl-2026',name:'SCIE · Clarivate 개별 확인 (2026-09-16)',year:'2026',url:'https://mjl.clarivate.com/',rows:[
  {venue:'Scientific Reports',issn:'2045-2322',alias:''},
...
export const builtinReferences:Reference[]=[builtinBk,builtinIeee,
  ...
  publisherReference('scie-apin-2026','Applied Intelligence','0924-669X;1573-7497','https://link.springer.com/journal/10489'),builtinMjl];
...
export const defaultCriteria={selected:builtinBk.id,custom:[] as Reference[],automatic:true,automaticSources:builtinReferences.map(r=>r.id),sourceRevision:1};
export const criteriaSchema=z.object({selected:z.string().min(1).max(100),custom:z.array(referenceSchema).max(10),automatic:z.boolean().default(true),automaticSources:z.array(z.string().min(1).max(100)).max(30).default(defaultCriteria.automaticSources),sourceRevision:z.number().int().min(0).max(1).default(0)}).refine(v=>criteriaOptions.some(o=>o.key===v.selected)||v.custom.some(r=>r.id===v.selected),'선택한 기준 자료가 없습니다.').refine(v=>new Set(v.custom.map(r=>r.id)).size===v.custom.length && v.custom.every(r=>!criteriaOptions.some(o=>o.key===r.id)),'기준표 식별자가 중복됩니다.').refine(v=>v.automaticSources.every(id=>builtinReferences.some(r=>r.id===id)||v.custom.some(r=>r.id===id)),'자동 대조 자료가 없습니다.').transform(v=>({
  ...v,sourceRevision:1,
  // Upgrade the old complete default set only; preserve deliberately restricted sources.
  automaticSources:v.sourceRevision===0 && builtinReferences.filter(r=>r!==builtinMjl).every(r=>v.automaticSources.includes(r.id))
    ? [...new Set([...v.automaticSources,builtinMjl.id])] : v.automaticSources,
...
export function journalLookupUrl(issn:string) {
  const id=issns(issn)[0];
  return id?'https://mjl.clarivate.com/search-results?issn='+id.slice(0,4)+'-'+id.slice(4)+'&hide_exact_match_fl=true':'https://mjl.clarivate.com/';
}
export function referenceEvidenceUrl(reference:Reference,row:Reference['rows'][number]) {return reference.id===builtinMjl.id?journalLookupUrl(row.issn):reference.url;}
type ReferenceRow=Reference['rows'][number];
const referenceIndexes=new WeakMap<Reference,{names:Map<string,ReferenceRow[]>;ids:Map<string,ReferenceRow[]>}>();
function referenceIndex(reference:Reference) {
  const cached=referenceIndexes.get(reference);if(cached)return cached;
  const names=new Map<string,ReferenceRow[]>(),ids=new Map<string,ReferenceRow[]>();
  const add=(map:Map<string,ReferenceRow[]>,key:string,row:ReferenceRow)=>{if(key)map.set(key,[...(map.get(key)||[]),row]);};
  for(const row of reference.rows) {
    for(const id of issns(row.issn))add(ids,id,row);
    for(const name of [row.venue,...row.alias.split(';').filter(a=>row.category!=='SCIE'||venueKey(a).length>8)])add(names,venueKey(name),row);
  }
  const result={names,ids};referenceIndexes.set(reference,result);return result;
}
...
export function applyReference(p:Paper,reference:Reference,row:Reference['rows'][number]):Paper {
  ...
  if(row.category==='SCIE')evidence.scie={status:'yes',year:reference.year,url:referenceEvidenceUrl(reference,row),note};
