...
export type Paper = KindInfo & { id: string; authorId: string; doi: string; type: string; source: string; publicationDate: string; arxiv: boolean; warnings: string[]; role: string; verified: boolean; values: Record<FieldKey, string>; customValues?: Record<string, string>; evidence?: RecognitionEvidence };
...
export function toPaper(work: Work, author: Author, professor: string): Paper {
  ...
  Object.assign(values, {
    ...
    published: work.publication_date?.slice(0, 7) ?? String(work.publication_year ?? ''),
  ...
  return { id: shortId(author.id) + ':' + shortId(work.id), authorId: author.id, doi: work.doi ?? '', type: work.type ?? '', source: work.id, values, verified: false, publicationDate: work.publication_date || String(work.publication_year || ''), arxiv: isArxiv(work), ...openAlexKind(work),
    role: [self?.author_position === 'first' ? '제1저자' : '', self?.is_corresponding ? '교신저자' : ''].filter(Boolean).join(' · ') || '제1·교신저자 정보 미확인',
    warnings: [work.is_authors_truncated ? '저자 목록이 일부 생략되어 있습니다. 전체 저자와 저자수를 원문에서 확인하세요.' : '', !first ? '제1저자 순서 정보가 없습니다. 원문 확인이 필요합니다.' : ''].filter(Boolean) };
}
