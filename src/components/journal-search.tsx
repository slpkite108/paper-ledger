...
import { authorQuery, candidateMatch, candidateToPaper, registerJournal, searchJournal, journalResult, journalResultKey, journalCandidateKey, sameJournalPaper, rebaseJournalResult, type JournalCandidate, type JournalSource, type JournalResult } from '@/lib/journal-search';
import { publicationRangeError, publicationRangeLabel, type PublicationRange } from '@/lib/publication-range';
import { dateBasisLabels, withDateBasis, dateSummary, type DateBasis } from '@/lib/publication-dates';
import { safeUrl, type Author, type Paper } from '@/lib/papers';

type Result=JournalResult;
export function JournalSearch({automaticResults,existingPapers,dateBasis,authors,professorNames,sources,onSources,range,includeUnknownMonths,onAdd,disabled}:{automaticResults:JournalResult[];existingPapers:Paper[];dateBasis:DateBasis;authors:Author[];professorNames:Record<string,string>;sources:JournalSource[];onSources:(s:JournalSource[])=>void;range:PublicationRange;includeUnknownMonths:boolean;onAdd:(p:Paper[],range:PublicationRange)=>void;disabled:boolean}) {
  ...
  const automatic=automaticResults.map(r=>rebaseJournalResult(r,dateBasis));
  const visibleResults=(manual?results.map(r=>rebaseJournalResult(r,dateBasis)):automatic).filter(r=>authors.some(a=>a.id===r.author.id));
  ...
  const pending=automatic.reduce((n,r)=>n+r.rows.filter(c=>!present(r,c)).length,0);
  const failures=automatic.filter(r=>r.error).length;
  const outside=automatic.reduce((n,r)=>n+r.excluded.length,0);
  ...
  async function run(more?:Result){
    ...
    for(const source of targets){
      ...
      try{
        ...
        const result=journalResult(source,author,term,period,unknown,page,dateBasis);
